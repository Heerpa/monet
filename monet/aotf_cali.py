...
def sweep_freq(aotf, powermeter, channel, freqs, t_wait=0.05):
...
def sweep_pdb(aotf, powermeter, channel, pdbs, t_wait=0.05):
...
def calibrate_all(instrument, protocol, powermeter):
    """Calibrate all defined channels.
    ...
    protocol : dict
        The calibration protocol (laser powers, beampath positions).
    powermeter : ThorlabsPowerMeter instance
    ...
    for las in lasers:
        ...
        if las in wavelengths:
            calibrate_lasers.append(las)
        else:
    ...
    for laser in calibrate_lasers:
        ...
        freqs = np.arange(
            prev_freq - freqwindow / 2,
            prev_freq + freqwindow / 2,
            step=freqstep,
        )
        pdbs = np.arange(0, 22.6, step=0.1)

        powers_f = sweep_freq(
            aotf.lowlvl, powermeter, channels[laser], freqs, t_wait=0.01
        )
        ...
        powers_p = sweep_pdb(
            aotf.lowlvl, powermeter, channels[laser], pdbs, t_wait=0.01
        )

...
class TestLocalCacheCalibrations(unittest.TestCase):
    ...
    def test_upsert_and_query_latest(self):
        ...
        self.cache.upsert_calibration(self._make_record(date='2024-01-02', bkg=2.0))
    ...
    def test_query_all(self):
        self.cache.upsert_calibration(self._make_record(date='2024-01-01', bkg=1.0))
        self.cache.upsert_calibration(self._make_record(date='2024-01-02', bkg=2.0))
        records = self.cache.query_calibrations({'name': 'TestScope'}, time_idx='all')
    ...
    def test_empty_query_returns_empty_list(self):
        records = self.cache.query_calibrations({'name': 'NoSuchScope'}, time_idx='all')
...
class TestLocalCacheOutbox(unittest.TestCase):
    ...
    def test_remove_entry(self):
        eid = self.cache.add_to_outbox('/calibrations', {'index': {}, 'parameters': {}})
    ...
    def test_pending_count(self):
        self.assertEqual(self.cache.pending_outbox_count(), 0)
        self.cache.add_to_outbox('/calibrations', {'index': {}, 'parameters': {}})
    ...
    def test_no_local_key(self):
        eid = self.cache.add_to_outbox('/calibrations/delete', {'device_name': 'S'})
        pending = self.cache.get_pending_outbox()
